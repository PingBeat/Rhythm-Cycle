...
@onready var coffee_record := $TVContainer/LevelsBox/CoffeeCycleButton/Record
@onready var office_record := $TVContainer/LevelsBox/OfficeCycleButton/Record
@onready var beach_record := $TVContainer/LevelsBox/BeachCycleButton/Record

@onready var btn_retour := $BottomBar/RetourBox/BtnRetour
@onready var btn_settings := $BottomBar/SettingsBox/BtnSettings

func _ready() -> void:
	# --- 2. MÉMOIRE DU MENU POUR LE BOUTON RETOUR ---
	Levelmanager.scene_retour_settings = "res://UI/select_level.tscn"

	# --- 3. AFFICHAGE DES RECORDS ---
	# On récupère les scores depuis le dictionnaire du Levelmanager
	coffee_record.text = "Record : " + str(Levelmanager.meilleurs_scores[1]) + " pts"
	office_record.text = "Record : " + str(Levelmanager.meilleurs_scores[2]) + " pts"
	beach_record.text = "Record : " + str(Levelmanager.meilleurs_scores[3]) + " pts"

	# --- 4. CONNEXIONS DES SIGNAUX ---
	# Clics niveaux
	...
	# Hover niveaux (on utilise Color("b2b2b2") pour être raccord avec le reste)
...
# --- 7. EFFETS VISUELS (HOVER) ---
func _on_button_hover(btn: Control) -> void:
	btn.modulate = Color("b2b2b2")

func _on_button_exit(btn: Control) -> void:
	btn.modulate = Color("ffffff")

func _on_bottom_hover(btn: Control) -> void:
	btn.modulate = Color("b2b2b2")

func _on_bottom_exit(btn: Control) -> void:
	btn.modulate = Color("ffffff")
